...
class Items(object):
    ...
    def sonic_screwdriver(self):
        ...
        # problem! Can't remove or add to tuple!
    ...
    def guard(self):
        print("A guard is blocking your path")
    def jail(self):
...
class Character(object):
    ...
    def examine(self):
        if self.location.item == "Sonic Screwdriver":
            Items.sonic_screwdriver(self)
        if self.location.item == "Tardis":
            Items.tardis(self)
        if self.location.item == "Tablet":
            Items.tablet(self)
        if self.location.item == "Guard":
            Items.guard(self)
        if self.location.item == "Jail":
            Items.jail(self)
        if self.location.item == "Door":
            Items.door(self) 
...
doctor = Character("The Doctor", room3)
