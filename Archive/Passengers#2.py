...
class Items(object):
    ...
    def use_teleporters(self):
        ...
        if "Clara" in doctor.inv and teleport_species == 0:
            teleport_species = 1
            input("Hundreds of species crowd onto the teleportation grid.")
            input("You key in the controls and set it to the nearest planet.")
            print("...")
            time.sleep(2)
            input("The teleportation is successful.")
            input("CLARA: 'Let's get back to the Tardis'")
            input("DOCTOR: 'We need to head to Main Control first'")
        else:
    ...
    def use_control_panel(self):
        ...
        if "Clara" in doctor.inv and teleport_species == 1 and return_ship == 0:
            return_ship = 1
            input("You boot up the main controls.")
            input("You set the ship to return to its origin")
            input("COUNTDOWN: 3 minutes")
            input("You send a message to the rest of the fleet: ")
            input("The Doctor is watching...")
            input("DOCTOR: Let's get back to the Tardis")
        else:
...
class Character(object):
    ...
    def room_descrip(self):
        if self.location.name == "Cargo Hold":
            ...
            print("You are in a room marked 'Main Controls'.")
        elif self.location.name == "Passenger Room":
            if "Clara" not in doctor.inv:
                input("You are in a room marked: 'PASSENGERS'.")
                input("You see rows after row of Stasis Pods.")
                input("Inside each one is a different species.")
                input("Silurians, Menoptera, Ood, Argolins, Hath...")
                input("All are ordered and catalogued.")
                input("A living museum.")
            else:
...
print ("\t\tDOCTOR WHO:")
print ("\t\tPASSENGERS\n\n")
input ("Clara has been kidnapped.")
input ("Fortunately, she was carrying your Sonic Screwdriver.")
...
while True:
    ...
    if command == "use sonic screwdriver":
        if "Sonic Screwdriver" in doctor.inv:
            if doctor.location == passenger and doctor.location.item == ["Pods"]:
                ...
                input("The 'passengers' begin to regain consciousness.")
                input("You tell them to head to the Teleportation Room")
    ...
    if command == "use control panel" or "use main control":
